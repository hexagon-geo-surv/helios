...
double
Triangle::getIncidenceAngle_rad(const glm::dvec3& rayOrigin,
...
inline double
Triangle::dotProductNaive(const glm::dvec3& v1, const glm::dvec3& v3)
{
  return v1.x * v3.x + v1.y * v3.y + v1.z * v3.z;
}

inline glm::dvec3
Triangle::crossProductNaive(const glm::dvec3& v1, const glm::dvec3& v2)
{
  return glm::dvec3(v1.y * v2.z - v1.z * v2.y,
                    v1.z * v2.x - v1.x * v2.z,
                    v1.x * v2.y - v1.y * v2.x);
}
 ...
 */
std::vector<double>
Triangle::getRayIntersection(const glm::dvec3& rayOrigin,
                             const glm::dvec3& rayDir)
{
  ...
  glm::dvec3 const s = rayOrigin - this->verts[0].pos;
  double const u = f * dotProductNaive(s, h);

  if (u < 0.0 || u > 1.0)
    return std::vector<double>{ -1 };

  glm::dvec3 const q = crossProductNaive(s, e1);
  double const v = f * dotProductNaive(rayDir, q);

  if (v < 0.0 || u + v > 1.0)
    return std::vector<double>{ -1 };

  double const t = f * dotProductNaive(e2, q);
...
double
Triangle::getRayIntersectionDistance(const glm::dvec3& rayOrigin,
                                     const glm::dvec3& rayDir)
{
...
void
Triangle::setAllVertexColors(Color4f color)
{
  verts[0].color = color;
  verts[1].color = color;
  verts[2].color = color;
...
void
Triangle::setAllVertexNormalsFromFace()
{
...
inline double
Triangle::euclideanDistance2D(const glm::dvec3& v1, const glm::dvec3& v2)
{
...
std::ostream&
operator<<(std::ostream& out, Triangle* t)
{
