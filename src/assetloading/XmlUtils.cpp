...
// ************************ //
Color4f
XmlUtils::createColorFromXml(tinyxml2::XMLElement* node)
{
  Color4f col;
  try {
...
XmlUtils::createParamsFromXml(tinyxml2::XMLElement* paramsNode)
{
...
Rotation
XmlUtils::createRotationFromXml(tinyxml2::XMLElement* rotGroupNode)
{
...
glm::dvec3
XmlUtils::createVec3dFromXml(tinyxml2::XMLElement* node, std::string attrPrefix)
{
  if (node == nullptr) {
...
ObjectT
XmlUtils::getAttribute(tinyxml2::XMLElement* element,
                       std::string attrName,
                       std::string type,
                       ObjectT defaultVal,
...
bool
XmlUtils::hasAttribute(tinyxml2::XMLElement* element, std::string attrName)
{
  return element->Attribute(attrName.c_str()) != nullptr;
}

bool
XmlUtils::isGeometryLoadingFilter(AbstractGeometryFilter* filter)
{
  if (dynamic_cast<WavefrontObjFileLoader*>(filter) != nullptr)
    return true;
  if (dynamic_cast<XYZPointCloudFileLoader*>(filter) != nullptr)
    return true;
  if (dynamic_cast<GeoTiffFileLoader*>(filter) != nullptr)
    return true;
  if (dynamic_cast<DetailedVoxelLoader*>(filter) != nullptr)
    return true;
  return false;
}

std::vector<std::shared_ptr<DynMotion>>
XmlUtils::createDynMotionsVector(tinyxml2::XMLElement* element)
{
  ...
  while (xmlMotion != nullptr) {
    tinyxml2::XMLAttribute const* typeAttr = xmlMotion->FindAttribute("type");
    ...
    bool selfMode = false;
    tinyxml2::XMLAttribute const* selfModeAttr =
      xmlMotion->FindAttribute("selfMode");
    if (selfModeAttr != nullptr && selfModeAttr->BoolValue()) {
      selfMode = true;
    }
    // Handle motion autoCRS flag
    double autoCRS = 0.0;
    tinyxml2::XMLAttribute const* autoCRSAttr =
      xmlMotion->FindAttribute("autoCRS");
    if (autoCRSAttr != nullptr)
      autoCRS = autoCRSAttr->DoubleValue();

    // Handle identity
    if (type == "identity") {
      dms.push_back(std::make_shared<DynMotion>(rm3f.makeIdentity(), selfMode));
    }
    // Handle translation
      ...
      dms.push_back(std::make_shared<DynMotion>(
        rm3f.makeTranslation(vec), selfMode, autoCRS));
      ...
      dms.push_back(
        std::make_shared<DynMotion>(rm3f.makeReflection(ortho), selfMode));
      ...
      dms.push_back(std::make_shared<DynMotion>(
        rm3f.makeGlideReflection(ortho, shift), selfMode));
    } else if (type == "rotation") {
      ...
      if (motionAttr != nullptr) {
        ...
        dms.push_back(std::make_shared<DynMotion>(
          rm3f.makeTranslation(-center), selfMode, autoCRS));
      }
      ...
      double angle = MathConverter::degreesToRadians(motionAttr->DoubleValue());
      dms.push_back(
        std::make_shared<DynMotion>(rm3f.makeRotation(axis, angle), selfMode));
      ...
      dms.push_back(std::make_shared<DynMotion>(
        rm3f.makeHelical(axis, angle, glide), selfMode));
    } else if (type == "rotsym") {
      motionAttr = xmlMotion->FindAttribute("axis");
      if (motionAttr == nullptr) {
        throw HeliosException("XmlUtils::createDynMotionsVector found a rotsym "
                              "motion with no axis attribute");
      }
      arma::colvec axis(motionAttr->Value());
      motionAttr = xmlMotion->FindAttribute("angle");
      ...
      if (motionAttr == nullptr) {
        dms.push_back(std::make_shared<DynMotion>(
          rm3f.makeRotationalSymmetry(axis, angle), selfMode));
      } else {
        arma::colvec center(motionAttr->Value());
        dms.push_back(std::make_shared<DynMotion>(
          rm3f.makeRotationalSymmetry(axis, angle, center), selfMode));
      }
    }
...
void
XmlUtils::assertDocumentForAssetLoading(tinyxml2::XMLDocument& doc,
                                        ...
                                        std::string const& caller)
{
  ...
  if (firstNode == nullptr || firstNode->ToDeclaration() == nullptr) {
    ...
    ss << "XML document '" << filename << "' at '" << path
       << "' does not start with an XML declaration (<?xml ...?>). Not a standard XML file.";
    logging::ERR(ss.str());
  ...
  if (std::string(root->Name()) != "document") {
    std::stringstream ss;
    ss << "XML document '" << filename << "' at '" << path
       << "' root element is <" << root->Name() << "> but expected <document>. Not a standard XML file.";
    logging::ERR(ss.str());
    std::stringstream ss2;
