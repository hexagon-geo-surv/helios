...
private:
  ...
  /**
   * @brief Serialize a \f$\mathbb{R}^{3}\f$ rigid motion factory to a
   *  stream of bytes
   * @tparam Archive Type of rendering
   * @param ar Specific rendering for the stream of bytes
   * @param version Version number for the \f$\mathbb{R}^{3}\f$ rigid motion
   *  factory
   */
  template<typename Archive>
  void serialize(Archive& ar, const unsigned int version)
  {
    boost::serialization::void_cast_register<RigidMotionR3Factory,
                                             RigidMotionFactory>();
...
public:
   ...
   *  \f$\mathbb{R}^{3}\f$
   */
  virtual RigidMotion makeHelicalX(double const theta,
                                   double const glide) const;
  /**
   ...
   *  \f$\mathbb{R}^{3}\f$
   */
  virtual RigidMotion makeHelicalY(double const theta,
                                   double const glide) const;
  /**
   ...
   *  \f$\mathbb{R}^{3}\f$
   */
  virtual RigidMotion makeHelicalZ(double const theta,
                                   double const glide) const;
  /**
