 ...
 * frames.</p> <p>Another example could be a telescope control application,
 * where the rotation would transform the sighting direction at rest into the
 * desired observing direction when the telescope is pointed towards an object
 * of interest. In this case the rotation transforms the direction at rest in a
 * topocentric frame into the sighting direction in the same topocentric frame.
 * This implies in this case the frame is fixed and the vector moves.</p> <p>In
 * many case, both approaches will be combined. In our telescope example, we
 ...
 *
 * <p>Since a rotation is basically a vectorial operator, several rotations can
 * be composed together and the composite operation <code>r = r<sub>1</sub> o
...
public:
   ...
   */
  void getAngles(RotationOrder const* order,
                 double& roll,
                 double& pitch,
                 double& yaw);
