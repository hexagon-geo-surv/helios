...
void
HelicopterPlatform::_clone(std::shared_ptr<Platform> p)
{
...
// *********************** //
void
HelicopterPlatform::prepareSimulation(int simFrequency_hz)
{
...
void
HelicopterPlatform::initLegManual()
{
  // Set directional attitude
  try {
    glm::dvec3 targetDirXY =
      cached_vectorToTarget_xy / cached_distanceToTarget_xy;
...
void
HelicopterPlatform::initLeg()
{
  cache_aligning = true;
}

bool
HelicopterPlatform::waypointReached()
{
...
void
HelicopterPlatform::updateStaticCache()
{
  Platform::updateStaticCache();
  cache_turnIterations = cached_currentAngle_xy / cfg_yawStepMagnitude;
  if (smoothTurn)
    computeTurnDistanceThreshold();
  else if (slowdownEnabled)
    computeNonSmoothSlowdownDist();
};

glm::dvec3
HelicopterPlatform::getCurrentDirection()
{
...
HelicopterPlatform::computeTurnDistanceThreshold()
{
  ...
  double sign =
    -Vectorial::shortestRotationSign(start, cached_targetToNextAngle_xy);
  double angle;
...
HelicopterPlatform::computeNonSmoothSlowdownDist()
{
  ...
  size_t const k =
    (size_t)std::floor(std::fabs(std::log(tau / sta)) / std::fabs(std::log(f)));
  ...
  for (size_t i = 0; i < k; ++i) {
    motionMagnitude += std::min(st * velocityNorm, movePerSec_m_stepMagnitude);
...
// ********************** //
void
HelicopterPlatform::doControlStep(int simFrequency_hz)
{
...
double
HelicopterPlatform::computeLiftSinkRate()
{
...
void
HelicopterPlatform::computeXYSpeed(int simFrequency_hz)
{
  // Prepare XY speed computation
  if (smoothTurn)
    speed_xy = getCurrentDirection();
  else
    speed_xy = cached_vectorToTarget;
  ...
  // Stabilize pitch
  if (stabilizePitch) {
    if (pitch > cfg_pitch_base)
      pitch -= cfg_pitchStepMagnitude;
    else if (pitch < cfg_pitch_base)
      pitch += cfg_pitchStepMagnitude;
  }
...
void
HelicopterPlatform::computeEngineForce(double zForceTarget)
{
...
void
HelicopterPlatform::computeRotationAngles(int simFrequency_hz)
{
  // Assure pitch is inside allowed boundaries
  if (pitch > cfg_max_pitch)
    pitch = cfg_max_pitch;
  else if (pitch < cfg_min_pitch)
    pitch = cfg_min_pitch;
...
HelicopterPlatform::computeAlignmentAngles()
{
  ...
  double targetYaw = -Vectorial::directionToAngleXY(cached_vectorToTarget_xy);

  // Translate current yaw to [0, 2pi)
  if (yaw < 0.0)
    yaw += std::ceil((-yaw / PI_2)) * PI_2;
  if (yaw > PI_2)
    yaw -= std::floor(yaw / PI_2) * PI_2;
...
void
HelicopterPlatform::computeTurningAngles()
{
  try {
    // Determine rotation sign
    double sign;
    double currentDirXyAngle =
      Vectorial::directionToAngleXY(cached_dir_current_xy, true);
    ...
    if (!std::isnan(cached_targetToNextDir_xy.x) &&
        !std::isnan(cached_targetToNextDir_xy.y)) {
      sign = Vectorial::shortestRotationSign(currentDirXyAngle,
    ...
    if (!stopAndTurn) {
      ...
      if (roll > cfg_max_roll_offset)
        roll = cfg_max_roll_offset;
      if (roll < -cfg_max_roll_offset)
        roll = -cfg_max_roll_offset;
...
void
HelicopterPlatform::rotate(double roll, double pitch, double yaw)
{
  try {
    // Platform attitude
...
void
HelicopterPlatform::handleRoute(int simFrequency_hz)
{
  if (!cache_turning &&
