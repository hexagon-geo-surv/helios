...
void
MovingPlatform::_clone(std::shared_ptr<Platform> p)
{
  Platform::_clone(p);
...
// *********************** //
void
MovingPlatform::applySettings(std::shared_ptr<PlatformSettings> settings,
                              bool manual)
{
...
void
MovingPlatform::doSimStep(int simFrequency_hz)
{
...
MovingPlatform::initLegManual()
{
  ...
  double angle =
    glm::angle(glm::normalize(cached_vectorToTarget_xy), cached_dir_current_xy);
