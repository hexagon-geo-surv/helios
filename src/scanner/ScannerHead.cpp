...
// *********************** //
void
ScannerHead::applySettings(std::shared_ptr<ScannerSettings> settings)
{
  ...
  this->cfg_setting_rotateRange_rad =
    std::fabs(cfg_setting_rotateStart_rad - cfg_setting_rotateStop_rad);
}
void
ScannerHead::doSimStep(double pulseFreq_Hz)
{
  if (cfg_setting_rotatePerSec_rad != 0) {
    setCurrentRotateAngle_rad(state_currentRotateAngle_rad +
                              cfg_setting_rotatePerSec_rad / pulseFreq_Hz);
  }
}

bool
ScannerHead::rotateCompleted()
{
  bool result = false;
...
void
ScannerHead::setCurrentRotateAngle_rad(double angle_rad)
{
...
void
ScannerHead::setRotatePerSec_rad(double rotateSpeed_rad)
{
