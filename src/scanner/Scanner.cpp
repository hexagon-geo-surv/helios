...
Scanner::Scanner(Scanner& s)
{
  ...
  if (s.allOutputPaths == nullptr)
    ...
    this->allOutputPaths =
      std::make_shared<std::vector<std::string>>(*s.allOutputPaths);
  }
  if (s.allMeasurements == nullptr)
    this->allMeasurements = nullptr;
  else {
    this->allMeasurements =
      std::make_shared<std::vector<Measurement>>(*s.allMeasurements);
  }
  if (s.allTrajectories == nullptr)
    this->allTrajectories = nullptr;
  else {
    this->allTrajectories =
      std::make_shared<std::vector<Trajectory>>(*s.allTrajectories);
  }
  if (s.allMeasurementsMutex == nullptr)
    this->allMeasurementsMutex = nullptr;
  else
...
// ********************* //
void
Scanner::_clone(Scanner& sc) const
{
  ...
  if (allOutputPaths == nullptr) {
    ...
    sc.allOutputPaths =
      std::make_shared<std::vector<std::string>>(*allOutputPaths);
  }
  if (allMeasurements == nullptr) {
    sc.allMeasurements = nullptr;
  } else {
    sc.allMeasurements =
      std::make_shared<std::vector<Measurement>>(*allMeasurements);
  }
  if (allTrajectories == nullptr) {
    sc.allTrajectories = nullptr;
  } else {
    sc.allTrajectories =
      std::make_shared<std::vector<Trajectory>>(*allTrajectories);
  }
  ...
  if (intersectionHandlingNoiseSource == nullptr) {
    ...
    sc.intersectionHandlingNoiseSource =
...
Scanner::retrieveCurrentSettings(size_t const idx)
{
...
void
Scanner::setPulseFreq_Hz(int pulseFreq_Hz)
{
...
// ************************ //
void
Scanner::handleSimStepNoise(glm::dvec3& absoluteBeamOrigin,
                            Rotation& absoluteBeamAttitude)
{
  ...
  if (!platformNoiseDisabled) {
    if (platform->attitudeXNoiseSource != nullptr) { // Add attitude X noise
      absoluteBeamAttitude = absoluteBeamAttitude.applyTo(
        Rotation(Directions::right, platform->attitudeXNoiseSource->next()));
    }
    if (platform->attitudeYNoiseSource != nullptr) { // Add attitude Y noise
      absoluteBeamAttitude = absoluteBeamAttitude.applyTo(
        Rotation(Directions::forward, platform->attitudeYNoiseSource->next()));
    }
    if (platform->attitudeZNoiseSource != nullptr) { // Add attitude Z noise
      absoluteBeamAttitude = absoluteBeamAttitude.applyTo(
        Rotation(Directions::up, platform->attitudeZNoiseSource->next()));
    }
  }
}
...
Scanner::handleTrajectoryOutput(double const currentGpsTime, Scene& scene)
{
  ...
  if (!platform->canMove())
    platform->writeNextTrajectory = false;
}

void
Scanner::trackOutputPath(std::string const& path)
{
  if (allOutputPaths != nullptr) {
    std::unique_lock<std::mutex> lock(*allMeasurementsMutex);
...
Scanner::initializeSequentialGenerators()
{
  ...
  randGen1->computeNormalDistribution(0.0,
                                      getDetector()->cfg_device_accuracy_m);
  randGen2->computeNormalDistribution(0.0, 1.0);
  intersectionHandlingNoiseSource =
    std::make_shared<UniformNoiseSource<double>>(*DEFAULT_RG, 0.0, 1.0);
