...
void
MultiScanner::_clone(Scanner& sc) const
{
                        ...
                        Scene& scene)
{
  ...
  glm::dvec3 const absoluteMountPosition = platform->getAbsoluteMountPosition();
  Rotation const absoluteMountAttitude = platform->getAbsoluteMountAttitude();
  for (size_t i = 0; i < numScanDevs; ++i) {
...
{
  scanDevs[idx].computeSubrays(handleSubray,
