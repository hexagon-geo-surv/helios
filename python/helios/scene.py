...
class StaticScene(Model, cpp_class=_helios.StaticScene):
    ...
    def from_binary(cls, binary_file: AssetPath):
        if not hasattr(_helios.StaticScene, "from_binary"):
            raise RuntimeError(
                "Native StaticScene binary loading is unavailable. "
                "Please rebuild the _helios extension."
            )
    ...
    def to_binary(self, binary_file: Path, compression_level: CompressionLevel = 6):
        ...
        self._finalize()
        if not hasattr(self._cpp_object, "to_binary"):
            raise RuntimeError(
                "Native StaticScene binary writing is unavailable. "
                "Please rebuild the _helios extension."
            )
